...
async def list_questions(topic_id: int, db: AsyncSession = Depends(get_db)):
...
async def generate_questions(payload: QuestionGenerateRequest, db: AsyncSession = Depends(get_db)):
...
async def create_question_manual(payload: QuestionCreateManual, db: AsyncSession = Depends(get_db)):
    topic = await db.get(Topic, payload.topic_id)
    if topic is None:
        raise HTTPException(status_code=404, detail="Topic not found")
...
async def external_prompt(topic_id: int, difficulty: str = "medium", count: int = 5, db: AsyncSession = Depends(get_db)):
